...
MONGO_URL = os.getenv(
    "MONGO_URL", "mongodb://admin:admin123@mongodb:27017/chaters_db?authSource=admin"
)
        ...
        await items_collection.insert_many(
            [
                ...
                },
                {
                    ...
                    "price": 25.0,
                    "description": "Chuột Silent công thái học",
                },
            ]
        )
...
async def read_all_items():
    tid = get_trace_id()
    logger.info(
        f"[{tid}] [READ_ALL] Người dùng yêu cầu lấy toàn bộ danh sách sản phẩm."
    )
...
async def read_item(item_id: int):
    ...
    if not item:
        logger.warning(
            f"[{tid}] [READ_ONE] Thất bại - Không tìm thấy sản phẩm ID: {item_id}"
        )
...
async def create_item(item: Item):
    ...
    logger.info(
        f"[{tid}] [CREATE] Thành công - Tạo sản phẩm mới ID: {new_id}, Name: {item.name}"
    )
...
async def update_item(item_id: int, item: Item):
    ...
    if result.matched_count == 0:
        logger.error(
            f"[{tid}] [UPDATE] Thất bại - Không tìm thấy ID {item_id} để cập nhật"
        )
...
async def delete_item(item_id: int):
    ...
    if not item:
        logger.warning(
            f"[{tid}] [DELETE] Thất bại - Không thể xóa ID {item_id} vì không tồn tại"
        )
        raise HTTPException(
            status_code=404, detail="Không thể xóa sản phẩm không tồn tại"
        )

    await items_collection.delete_one({"_id": item_id})
    logger.info(
        f"[{tid}] [DELETE] Thành công - Đã xóa sản phẩm ID: {item_id} ({item['name']})"
    )
